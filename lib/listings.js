...
export const createListing = async (restaurantId, itemName, quantity, expiresAt, imageUrl = null, description = null) => {
  try {
...
export const getAllListings = async () => {
  try {
    ...
    // Ensure status is correctly set based on claims
    const processedListings = data?.map(listing => ({
      ...listing,
      status: listing.claims && listing.claims.length > 0 ? 'claimed' : 'available'
    })) || [];
